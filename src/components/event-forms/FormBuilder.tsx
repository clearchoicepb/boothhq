            ...
            <div className="space-y-2">
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  name="scope"
                  checked={!editedField.scope || editedField.scope === 'shared'}
                  onChange={() => setEditedField({ ...editedField, scope: 'shared' })}
                  className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700">Shared (same for all dates)</span>
              </label>
              <label className="flex items-center gap-2">
                <input
                  type="radio"
                  name="scope"
                  checked={editedField.scope === 'per_date'}
                  onChange={() => setEditedField({ ...editedField, scope: 'per_date' })}
                  className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700">Per Date (can differ by date)</span>
              </label>
              <p className="text-xs text-gray-500 mt-1">
                Per-date fields appear in each date&apos;s section on the public form for multi-day events.
              </p>
            </div>
          </div>
        )}
