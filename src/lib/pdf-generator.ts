...
export const generateQuotePDF = async (data: QuotePDFData): Promise<Buffer> => {
  ...
  doc.setFontSize(24)
  doc.setFont('helvetica', 'bold')
  doc.text('QUOTE', rightEdge, yPos + 8, { align: 'right' })

  doc.setFontSize(12)
  doc.setFont('helvetica', 'normal')
  doc.text(`#${quote.quote_number}`, rightEdge, yPos + 15, { align: 'right' })
  ...
  if (quote.account_name) {
    doc.text(quote.account_name, margin, yPos)
    yPos += 5
  }
  ...
  if (quote.contact_name && quote.account_name) {
    doc.text(`Attn: ${quote.contact_name}`, margin, yPos)
    yPos += 5
  } else if (quote.contact_name) {
    doc.setFontSize(11)
    doc.setFont('helvetica', 'bold')
    doc.text(quote.contact_name, margin, yPos)
    yPos += 5
    doc.setFontSize(9)
    doc.setFont('helvetica', 'normal')
  }

  if (quote.opportunity_name) {
    doc.text(`RE: ${quote.opportunity_name}`, margin, yPos)
    yPos += 5
  }
  ...
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(0, 0, 0)
  doc.text(formatDate(quote.issue_date), rightColX, rightColY)
  rightColY += 8
  ...
  if (quote.valid_until) {
    ...
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(100, 100, 100)
    doc.text('VALID UNTIL', rightColX, rightColY)
    rightColY += 5

    doc.setFontSize(10)
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(0, 0, 0)
    doc.text(formatDate(quote.valid_until), rightColX, rightColY)
    rightColY += 8
  }
  ...
  quote.line_items.forEach((item, index) => {
    ...
    // Draw subtle separator line between items
    if (index < quote.line_items.length - 1) {
      doc.setDrawColor(240, 240, 240)
      doc.setLineWidth(0.1)
      doc.line(margin, yPos - 3, rightEdge, yPos - 3)
    }
  ...
  doc.setFont('helvetica', 'bold')
  doc.setTextColor(0, 0, 0)
  doc.text(formatCurrency(quote.subtotal), rightEdge, yPos, { align: 'right' })
  yPos += 7

  if (quote.tax_amount && quote.tax_amount > 0) {
    doc.setFont('helvetica', 'normal')
    doc.setTextColor(70, 70, 70)
    doc.text(`Tax (${((quote.tax_rate || 0) * 100).toFixed(2)}%):`, totalsX, yPos)

    doc.setFont('helvetica', 'bold')
    doc.setTextColor(0, 0, 0)
    doc.text(formatCurrency(quote.tax_amount), rightEdge, yPos, { align: 'right' })
    yPos += 7
  }
  ...
  doc.text('Total:', totalsX, yPos)
  doc.setFontSize(14)
  doc.text(formatCurrency(quote.total_amount), rightEdge, yPos, { align: 'right' })
