...
async function seedDefaultTypesIfNeeded(
  ...
  log.debug({ tenantId: dataSourceTenantId }, 'seedDefaultTypesIfNeeded called')
  ...
  log.debug({ tenantId: dataSourceTenantId }, 'Seeding default types for tenant')
  ...
  log.debug({ firstType: typesToInsert[0] }, 'First type to insert')
  ...
  if (error) {
    ...
    log.debug({ insertedCount: insertedData?.length }, 'Inserted data count')
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    log.debug({ typeId: data?.id }, 'Successfully created type')
