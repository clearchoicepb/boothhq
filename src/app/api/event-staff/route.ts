...
export async function GET(request: NextRequest) {
  ...
  const { supabase, dataSourceTenantId, session } = context
    ...
    log.debug({ dataSourceTenantId, eventId }, 'Starting query for tenant')
    ...
    log.debug({ count: data?.length || 0 }, 'Success, found records')
...
export async function POST(request: NextRequest) {
  try {
    ...
    log.debug({ email: session.user.email, dataSourceTenantId }, 'User authenticated')
