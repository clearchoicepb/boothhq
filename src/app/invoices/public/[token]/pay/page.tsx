...
function PaymentForm({ invoice, token, clientSecret, onSuccess }: PaymentFormProps) {
...
export default function PublicInvoicePaymentPage() {
  ...
  const createPaymentIntent = async (amount?: number) => {
    try {
      ...
      const response = await fetch(
        `/api/public/invoices/${token}/pay?amount=${amountToUse}`
      )
  ...
  const handlePaymentSuccess = () => {
    router.push(`/invoices/public/${token}?paid=true`)
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Card className="max-w-md w-full mx-4">
          ...
          <CardContent>
            <Link href={`/invoices/public/${token}`}>
